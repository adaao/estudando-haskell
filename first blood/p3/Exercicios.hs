...
data C a = UmaC a | DuasC a a | TresC a a a | N deriving Show

instance Functor C where
  fmap f N = N
  fmap f (UmaC a) = UmaC (f a)
  fmap f (DuasC a b) = DuasC (f a) (f b)
  fmap f (TresC a b c) = TresC (f a) (f b) (f c)

instance Applicative C where
  pure = N
  N <*> _ = N
  _ <*> N = N
  UmaC (f) <*> UmaC (a) = UmaC (f a)
